        ...
        {isProcessing ? (
          <Card>
            <CardContent className='flex flex-col items-center gap-3 py-16'>
              ...
              <p className='text-xs text-muted-foreground'>Matching invoice lines to contract terms</p>
            </CardContent>
          </Card>
        ) : audit ? (
          <>
            <div className='grid grid-cols-3 gap-4'>
              <SummaryCard label='Invoice Total' value={formatCurrency(audit.invoiceTotal)} />
              <SummaryCard label='Expected Total' value={formatCurrency(audit.expectedTotal)} sub='per contract' />
              <SummaryCard
                label='Variance'
            ...
            <Card>
              <CardHeader>
                ...
                <p className='text-sm text-muted-foreground'>Click a row with a variance to see evidence</p>
              </CardHeader>
                    ...
                    {audit.lineResults.map((line: AuditLineResult) => (
                      <tr
                        ...
                        className={`border-b last:border-0 transition-colors ${
                          line.result !== 'ok' ? 'cursor-pointer hover:bg-muted/30' : ''
                        } ${selectedLine?.id === line.id ? 'bg-primary/5' : ''}`}
                        onClick={() =>
                          line.result !== 'ok' && setSelectedLine(selectedLine?.id === line.id ? null : line)
                        }
                      >
                        <td className='px-6 py-3'>
                          <p className='font-medium'>{line.description}</p>
                        </td>
                        <td className='px-6 py-3 text-right font-mono'>{formatCurrency(line.invoicedAmount)}</td>
                        <td className='px-6 py-3 text-right font-mono'>{formatCurrency(line.expectedAmount)}</td>
                        <td
          ...
          <Card>
            <CardContent className='py-8 text-center text-muted-foreground'>No audit result available.</CardContent>
          </Card>
        )}
      </div>
...
function EvidencePanel({ line, onClose }: { line: AuditLineResult; onClose: () => void }) {
  let evidence: AuditEvidence | null = null;
  try {
    evidence = typeof line.evidence === 'string' ? JSON.parse(line.evidence) : (line.evidence as AuditEvidence);
  } catch {
    evidence = null;
  }
  ...
  return (
    <div className='w-96 border-l bg-background overflow-auto'>
      ...
      <div className='p-6 space-y-6'>
        {/* Line summary */}
        <div>
          <p className='text-xs font-medium uppercase tracking-wide text-muted-foreground mb-2'>Charge</p>
          <p className='font-medium'>{line.description}</p>
          <div className='mt-2 flex items-center gap-2'>
            <LineResultBadge result={line.result} />
            <span className={`text-sm font-medium ${line.variance > 0 ? 'text-destructive' : 'text-emerald-600'}`}>
              {line.variance > 0 ? 'Overcharged by' : 'Undercharged by'} {formatCurrency(Math.abs(line.variance))}
            </span>
          </div>
        </div>
        ...
        <div className='rounded-lg border border-destructive/20 bg-destructive/5 p-4'>
          <div className='flex items-center gap-2 mb-2'>
            ...
            <p className='text-xs font-semibold uppercase tracking-wide text-destructive'>Invoice Says</p>
          </div>
        ...
        {evidence?.source && (
          <div className='rounded-lg border bg-muted/40 p-4'>
            <p className='text-xs font-medium uppercase tracking-wide text-muted-foreground mb-1'>Source</p>
        ...
        {/* Explanation */}
        <div>
          <p className='text-xs font-medium uppercase tracking-wide text-muted-foreground mb-2'>Explanation</p>
          <p className='text-sm text-muted-foreground leading-relaxed'>{line.explanation}</p>
        </div>
  ...
  if (status === 'overcharge')
    return (
      <div className='flex items-center gap-2 rounded-lg border border-destructive/30 bg-destructive/10 px-3 py-2'>
        ...
        <span className='text-sm font-semibold text-destructive'>Overcharge Detected</span>
      </div>
    );
  if (status === 'ok')
    return (
      <div className='flex items-center gap-2 rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2'>
        ...
        <span className='text-sm font-semibold text-emerald-700'>Invoice Verified</span>
      </div>
    );
  return (
