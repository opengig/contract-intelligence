...
import { Plus, FileText, X, ExternalLink, ChevronDown, ChevronRight, Loader2 } from 'lucide-react';
...
export default function InvoicesPage() {
  ...
  return (
    <div className='flex h-full'>
      ...
      <div className='flex-1 min-w-0 overflow-auto p-8 space-y-6'>
        <div className='flex items-center justify-between'>
          <div>
            ...
            <p className='text-muted-foreground mt-1'>Upload and audit vendor invoices against contract terms</p>
          </div>
          <Button asChild>
                ...
                <tbody>
                  {invoices?.map((inv) => {
                    ...
                    return (
                      ...
                      >
                        <td className='px-6 py-3 font-mono font-medium'>{inv.invoiceNo}</td>
                        <td className='px-6 py-3 text-muted-foreground'>{(inv as any).vendor?.name ?? '—'}</td>
                        <td className='px-6 py-3'>{audit ? formatCurrency(audit.invoiceTotal) : '—'}</td>
                        ...
                        <td className='px-6 py-3 text-muted-foreground text-xs'>{resultSummary(lines)}</td>
      ...
      {/* Invoice sidesheet */}
      {selectedId && <InvoiceSidesheet invoiceId={selectedId} onClose={() => setSelectedId(null)} />}
    </div>
  );
}

function InvoiceSidesheet({ invoiceId, onClose }: { invoiceId: string; onClose: () => void }) {
      ...
      <div className='sticky top-0 flex items-center justify-between border-b bg-background px-5 py-4'>
        <div className='flex-1 min-w-0 mr-3'>
          {isLoading ? (
            ...
            <>
              <p className='font-semibold font-mono truncate'>{invoice?.invoiceNo}</p>
              <p className='text-xs text-muted-foreground truncate'>{(invoice as any)?.vendor?.name}</p>
            </>
          )}
        </div>
          ...
          <>
            ...
            {audit ? (
              <div className='grid grid-cols-3 divide-x border-b'>
                <SummaryCell label='Invoice Total' value={formatCurrency(audit.invoiceTotal)} />
                <SummaryCell
                ...
                <SummaryCell
                  label='Variance'
                  value={
                    audit.variance > 0 ? `+${formatCurrency(audit.variance)}` : formatCurrency(Math.abs(audit.variance))
                  }
              ...
              <p className='px-5 py-6 text-sm text-center text-muted-foreground'>No audit results yet.</p>
            )}
            ...
            {audit && (
              <div className='flex items-center justify-between px-5 py-3 border-b bg-muted/20'>
                ...
                <span className='text-xs text-muted-foreground font-mono truncate max-w-48'>{invoice.fileName}</span>
              </div>
            )}
                ...
                <div className='divide-y'>
                  {lines.map((line: any) => {
                    ...
                    return (
                      <div key={line.id}>
                        <div
                          className={`px-5 py-3 ${hasVariance ? 'cursor-pointer hover:bg-muted/20' : ''}`}
                          onClick={() => hasVariance && setExpandedLine(isExpanded ? null : line.id)}
                        >
                            ...
                            <div className='flex items-center gap-1.5 flex-1 min-w-0'>
                              ...
                              <div className='min-w-0'>
                                <p className='text-sm font-medium leading-tight truncate'>
                                  {line.invoiceLine?.description ?? line.invoiceLine?.chargeType}
                                </p>
                              </div>
                            </div>
...
function EvidencePanel({ evidence, explanation }: { evidence: unknown; explanation?: string }) {
  ...
  return (
    <div className='mx-5 mb-3 rounded-md border bg-muted/30 p-3 text-xs space-y-2'>
      ...
      {ev.source && <EvidenceRow label='Source' value={ev.source} valueClass='text-muted-foreground' />}
      {explanation && <div className='pt-1 border-t text-muted-foreground italic'>{explanation}</div>}
    </div>
  );
}
