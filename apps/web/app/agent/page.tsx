          ...
          <div className="space-y-1 max-h-36 overflow-y-auto">
            {textMatches.map((r, i: number) => (
              <div key={i} className="rounded-md bg-background border border-border/50 p-2 space-y-0.5 text-xs">
                ...
                <p className="text-muted-foreground">{r.contractName} · {r.vendorName}{r.rate != null ? ` · ₹${r.rate}${r.unit ? `/${r.unit}` : ''}` : ''}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="space-y-1 max-h-36 overflow-y-auto">
            {semanticMatches.map((r, i: number) => (
              <div key={i} className="rounded-md bg-background border border-border/50 p-2 space-y-0.5 text-xs">
                <p className="font-medium truncate">{r.contractName} · {r.vendorName}</p>
                <p className="text-muted-foreground line-clamp-2">{r.content}</p>
                <div className="flex items-center gap-1.5">
                  <div className="h-1 w-16 rounded-full bg-muted overflow-hidden">
                    <div className="h-full rounded-full bg-violet-500" style={{ width: `${Math.round(Number(r.similarity) * 100)}%` }} />
                  </div>
                  <span className="text-[10px] text-muted-foreground/60">{(Number(r.similarity) * 100).toFixed(0)}% match</span>
                </div>
              </div>
            ))}
          </div>
...
function RunQueryResult({ result, input }: { result: Record<string, any>; input?: Record<string, any> }) {
  ...
  if (result?.error) {
    return (
      <div className="space-y-2">
        {sql && <pre className="text-xs font-mono bg-background border border-border/50 rounded-md p-2 overflow-x-auto whitespace-pre-wrap">{sql}</pre>}
  ...
  return (
    <div className="space-y-2">
      {sql && (
        <pre className="text-xs font-mono bg-background border border-border/50 rounded-md p-2 overflow-x-auto whitespace-pre-wrap text-muted-foreground">{sql}</pre>
      )}
      <p className="text-xs text-muted-foreground">{count} row{count !== 1 ? 's' : ''} returned</p>
            ...
            <thead>
              <tr className="bg-muted/50 border-b">
                {Object.keys(rows[0]).map((key) => (
                  <th key={key} className="text-left font-medium text-muted-foreground px-2 py-1.5 whitespace-nowrap">{key}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {rows.slice(0, 10).map((row, i) => (
                <tr key={i} className="border-b border-border/30 last:border-0">
                  {Object.values(row).map((val, j) => (
                    <td key={j} className="px-2 py-1 whitespace-nowrap max-w-[200px] truncate">{val == null ? '—' : String(val)}</td>
                  ))}
                </tr>
              ))}
            </tbody>
          </table>
          {rows.length > 10 && <p className="text-[10px] text-muted-foreground px-2 py-1">…and {rows.length - 10} more rows</p>}
        </div>
      )}
    </div>
  );
...
function ListTablesResult({ result }: { result: Record<string, any> }) {
  ...
  return (
    <div className="space-y-1.5">
      <p className="text-xs text-muted-foreground">{tables.length} table{tables.length !== 1 ? 's' : ''} found</p>
        ...
        {tables.map((t, i: number) => (
          <div key={i} className="rounded-md bg-background border border-border/50 p-2 text-xs">
            <div className="flex items-center gap-2 mb-1">
              ...
              <Badge variant="secondary" className="text-[10px] shrink-0 ml-auto">{t.rowCount} rows</Badge>
            </div>
            ...
            {t.columns && (
              <div className="flex flex-wrap gap-1 mt-1.5 ml-5">
                {(t.columns as string[]).slice(0, 6).map((col: string) => (
                  <span key={col} className="rounded bg-muted px-1.5 py-0.5 text-[10px] font-mono text-muted-foreground">{col}</span>
                ))}
                {(t.columns as string[]).length > 6 && (
                  <span className="text-[10px] text-muted-foreground/60">+{(t.columns as string[]).length - 6} more</span>
                )}
              </div>
            )}
          </div>
          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              {sampleRows.map((row, i) => (
                <tr key={i} className="border-b border-border/30 last:border-0">
                  {Object.values(row).map((val, j) => (
                    <td key={j} className="px-2 py-1 whitespace-nowrap max-w-[150px] truncate">{val == null ? '—' : String(val)}</td>
                  ))}
                </tr>
              ))}
            </tbody>
    ...
    <div className="space-y-2">
      {code && (
        <details className="group/code">
          ...
          <pre className="mt-1.5 text-xs font-mono bg-zinc-950 text-zinc-100 rounded-md p-3 overflow-x-auto max-h-48 whitespace-pre-wrap">{code}</pre>
        </details>
      )}
      {stdout && (
...
function SearchTablesResult({ result }: { result: Record<string, any>[] | Record<string, any> }) {
  ...
  return (
    <div className="space-y-1.5">
      <p className="text-xs text-muted-foreground">{tables.length} table{tables.length !== 1 ? 's' : ''} matched</p>
...
function isToolPart(part: Record<string, any>): boolean {
  const type = part.type as string;
  return type === 'dynamic-tool' || (type?.startsWith('tool-') && type !== 'tool-invocation' && type !== 'tool-call' && type !== 'tool-result')
    || type === 'tool-invocation' || type === 'tool-call' || type === 'tool-result';
}
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function ToolResultContent({ toolName, result, input }: { toolName: string; result: Record<string, any>; input?: Record<string, any> }) {
      ...
      {steps.map((step, i) => {
        ...
        return (
          <span key={i} className="flex items-center gap-1">
            ...
            <span className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-medium transition-colors ${step.isDone ? 'bg-muted text-muted-foreground' : `${meta.bgColor} ${meta.color}`}`}>
              {step.isDone ? (
  ...
  useEffect(() => {
    contractsApi.getAll()
      .then((data) => {
        ...
        setContracts(list);
        const bollore = list.find((c) =>
          c.vendor.name.toLowerCase().includes('bollore') ||
          c.vendor.name.toLowerCase().includes('bolloré'),
        );
  ...
  const { messages, sendMessage, status, setMessages, stop } = useChat({
    transport: new DefaultChatTransport({
      ...
      body: selectedContractId
        ? { contractId: selectedContractId, vendorId: selectedContract?.vendorId }
        : {},
    }),
  });
        ...
        <Conversation className="flex-1 px-4">
          <ConversationContent className="mx-auto max-w-3xl">
            {messages.map((m, msgIdx) => {
              ...
              // eslint-disable-next-line @typescript-eslint/no-explicit-any
              type RenderItem = { kind: 'text'; part: Record<string, any>; idx: number } | { kind: 'tools'; parts: Record<string, any>[] };
                ...
                <Message key={m.id} from={m.role}>
                  <MessageContent>
                    ...
                    {renderItems.map((item, renderIdx) => {
                      ...
                      if (part.type === 'text') {
                        if (isUser) {
                          return <p key={partKey} className="text-sm whitespace-pre-wrap">{part.text}</p>;
                        }
                        return (
                          <MessageResponse
                            key={partKey}
    ...
    <div className="mx-auto flex max-w-2xl flex-col items-center gap-8 py-12 px-4">
      <div className="flex flex-col items-center gap-3 text-center">
        ...
        <p className="text-sm text-muted-foreground max-w-sm">
          I query your live contract database, calculate rates.
        </p>
